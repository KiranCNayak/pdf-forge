...
> route because the user intent is different — split means *divide this document*,
> extract means *give me these pages*. Organize is Hybrid only because its thumbnail grid
...
| Redact PDF | `/redact-pdf` | Go | Must *remove* the content stream text, not draw a black box over it. Anything less is a security defect |
...
> `/workflow` is worth flagging as an *advantage* rather than a chore. ihatepdf must
...
| --- | --- | --- |
| Chat with PDF | `/chat-with-pdf` | ihatepdf extracts text locally and posts it to Gemini. The *binary* never uploads, but **the document's contents do**. That is a materially weaker promise than ours, and their marketing doesn't distinguish the two |
...
| `/resources` | Blog index. ihatepdf runs ~90 SEO articles against these tools — worth noting as a *distribution* lesson even though it isn't engineering |
