...
| Flatten PDF       | `/flatten-pdf`      | Hybrid | **Shipped**, not as spec'd: `api.RemoveFormFields` deletes field objects, it never merges their appearance into the page first — a filled field would vanish, not flatten (confirmed directly in the vendored source). Same rasterize-and-rebuild architecture as Redact/Invert Colours instead. See `docs/tools/flatten.md`                                                                                                                       |
