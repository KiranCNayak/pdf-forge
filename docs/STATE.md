...
Next task`'s Redact entries for the detail; Redact is considered closed out. **Four more
Phase 4 tools shipped after that, by direct user request, in one pass**: Fingerprint
(zero new engine code — the third tool in a row to reuse `AddWatermarks`), Sign (the
first new Go op since Redact, `AddImageWatermark` — its own e2e test also caught and
fixed a real pdf.js Worker-compat bug shared by every render-worker tool, not just Sign;
see "Things that will bite you" #12), Invert Colours and Flatten (both reuse Redact's
full-page rasterize-and-rebuild architecture directly, for the same reason Redact
deviated from its own catalog spec — pdfcpu has no content-stream rewriter for colour
operators or form-field appearance baking either). Nothing is in flight — the next Phase
4 tool has not been chosen. The 20 remaining Phase 4 tools (`docs/TOOL_CATALOG.md`'s
Phase 4 section) all need real design work rather than a UI layer over a one-call API —
Fill Form, Edit PDF Text, Repair, Compare, Privacy Scanner, and the whole
office-format-conversion block. Ask the user which one before starting; don't assume the
order in the catalog is a priority order, it isn't.
...
| `web/src/tools/InvertColours`                                      | Redact's full-page rasterize-and-rebuild architecture reused directly (same deliberate deviation, same reason — pdfcpu has no content-stream colour-operator rewriter) with the compositing step replaced by a literal per-channel pixel invert instead of black boxes. A page left out of the selection is still rasterized (the whole document goes through the pipeline regardless) but keeps its original colours. Adapting Redact's `apply()` loop by hand copied its cancel-check as a plain `useState` boolean instead of a ref — the exact stale-closure bug Redact's own code comment warns about — caught by re-reading against that comment before it shipped, not by a failing test; fixed with a ref before commit, plus proactively applied Redact's other post-review fix (every input disabled during a run) to avoid rediscovering it the hard way a second time. `web/e2e/invert-colours.spec.ts` checks exact RGB arithmetic (a known solid colour inverts to a precise expected value), not a brightness threshold, since PNG output makes that possible here                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `web/src/tools/Flatten`                                            | Simplest of the four rasterize-and-rebuild tools — no box editor, no pixel transform, no page selection, and (unlike Redact/InvertColours) no decode→canvas→re-encode roundtrip either, since there's nothing to composite: the render worker's own encoded bytes go straight into `imagesToPDF`. Deviates from the catalog's own spec (`api.RemoveFormFields` + "annotation flattening") because the two don't compose — confirmed directly in the vendored source that `RemoveFormFields` only deletes field objects, never merges a filled field's appearance into the page first, so a filled value would vanish, not flatten. The load-bearing assumption (does pdf.js's render worker actually bake a filled field's value into the raster) was verified with a real fixture BEFORE writing any tool code — a new `-flatten-form` flag on `cmd/genfixtures` builds a real AcroForm text field via `api.Create` (pdfcpu's declarative JSON page format), rendered through PdfToImage and read back by eye before this tool existed, kept as a permanent `web/e2e/flatten.spec.ts` regression rather than a one-off manual check                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
120 Go tests pass. TypeScript is clean. Production build works. 62 Playwright e2e tests
across 24 spec files pass (`web/e2e/`, `npm run test:e2e`) — every tool has at least one.
`__smoke()` now has 12
