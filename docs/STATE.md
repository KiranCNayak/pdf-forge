...
- **Render-worker tools** — `web/src/lib/render/` and `render.worker.ts` exist (Lane D).
  `pdf-to-image` (per-page loop, cancel-via-ref) and `extract-text` (single call spanning
  every page, cancel-via-terminate) are both built and are the two reference shapes.
  `organize-pages` and `pdf-to-zip` still need it. `organize-pages` is the most involved
  (thumbnail grid, drag-reorder, local-state edits before Apply) — treat it as its own
  piece of work, not a variant of another tool page. `pdf-to-zip` is the first tool that
  needs an actual ZIP dependency — flag before adding one, per `docs/PARALLEL.md`.
