...
| `web/src/tools/P2PShare`                                           | Phase 3, first tool built on `signaling/`: `web/src/lib/p2p/` (`SignalingClient` — WS wrapper, push-driven not request/response like the other two clients; `PeerLink` — RTCPeerConnection + trickle ICE, buffers ICE candidates that race ahead of `setRemoteDescription`; `transfer.ts` — header/accept/reject/end control protocol over the data channel, chunked sending with `bufferedAmountLowThreshold` backpressure, SHA-256 verification). **V1 departures, documented in `transfer.ts`'s header, not silent:** whole file in memory rather than IndexedDB; no PBKDF2/AES-GCM password layer yet despite the doc calling it out as earning its place (transport is still DTLS-encrypted regardless); single file per transfer. Verified with two real browser tabs against a locally-run signaling server: full offer/answer/ICE handshake, a transferred file confirmed **byte-identical** to the original via `diff`, invalid-room-code and peer-declined error paths both correctly messaged, zero console errors throughout                                                                               |
...
**Phase 3 (P2P share) has a working V1.** `web/src/tools/P2PShare` and `web/src/lib/p2p/`
implement send/receive over a real WebRTC data channel, signaling via `signaling/`.
Verified with two live browser tabs against a locally-run signaling server: full offer/
answer/ICE handshake, a transferred file confirmed byte-identical via `diff`, invalid-code
and peer-declined error paths, zero console errors.

**What's left before this is done, not started:**

- **Password/encryption layer.** `docs/tools/p2p-share.md` calls this out as earning its
  place specifically because we introduced a signaling server (defends against a
  compromised server rewriting SDP fingerprints, not against passive eavesdropping — DTLS
  already covers that). PBKDF2-SHA256 → AES-256-GCM, same envelope as ihatepdf's own
  construction (which the doc says is sound). Deferred rather than rushed in this pass.
...
`images-to-pdf` is the one Phase-2-catalogued tool page still unbuilt — it may not need
the render worker at all (check whether it's closer to the Go-engine shape before
starting).
